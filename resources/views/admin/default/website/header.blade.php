		...
		<div class="card">
			...
			<div class="card-body">
				<form action="{{ route('system_configuration.update') }}" method="POST" enctype="multipart/form-data">
	                ...
	                <div class="form-group">
	                    <label class="form-label" for="signinSrEmail">{{ translate('Other Header Logo') }}</label>
	                    <div class="input-group " data-toggle="aizuploader" data-type="image">
	                        <div class="input-group-prepend">
	                            <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse') }}</div>
	                        </div>
	                        <div class="form-control file-amount">{{ translate('Choose File') }}</div>
							<input type="hidden" name="types[]" value="other_header_logo">
	                        <input type="hidden" name="other_header_logo" class="selected-files" value="{{get_setting('other_header_logo')}}">
	                    </div>
	                    <div class="file-preview"></div>
	                </div>
	                <div class="form-group">
						<label>{{ translate('Enable sticky header?') }}</label>
						<div>
