@foreach ($chats->reverse() as $chat)
    @if ($chat->sender_user_id == Auth::user()->id)
        @if ($chat->message != null)
            ...
            <div class="chat-coversation right d-flex align-items-end flex-column influencer-side chat-text text-grey {{ ($chat->as_proof)?'proof':'' }}">
                <div class="d-flex">
                ...
                <div class="time">{{ Carbon\Carbon::parse($chat->created_at)->diffForHumans() }}</div>
            </div>
        @endif
