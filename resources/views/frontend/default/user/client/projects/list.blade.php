							...
							<div class="card project-card">
								<div class="card-header border-bottom-0">
									...
									<div>
										...
										@elseif ($project->biddable == 1 && $project->multi_hires == 1)
											<span class="badge badge-info badge-inline badge-md">{{ translate('Running also Open') }}</span>
											<a href="javascript:void(0)" class="btn btn-primary btn-sm fw-500 confirm-complete" data-href="{{route('projects.hireDone', $project->id)}}">{{ translate('Stop Hiring') }}</a>
										@else
                                ...
                                <div class="card-footer">
									...
									@elseif ($project->biddable == 0 || ($project->biddable == 1 && $project->multi_hires == 1) )
										@foreach($project->project_user as $k=>$pro_user)
										<a href="{{ route('freelancer.details', $pro_user->user->user_name) }}" target="_blank" class="d-flex mr-3 align-items-center text-inherit" tabindex="0">
		                                    <span class="avatar avatar-xs">
                                                @if($pro_user->user->photo != null)
                                                    <img src="{{ custom_asset($pro_user->user->photo) }}">
                                                @else
                                                    <img src="{{ my_asset('assets/frontend/default/img/avatar-place.png') }}">
                                                @endif
		                                    </span>
		                                    <div class="pl-2">
		                                    	<h4 class="h6 mb-0 fs-14">{{ $pro_user->user->name }}</h4>
                                                <div class="">
    												<span class="bg-rating rounded text-white px-1 mr-1 fs-10">
                                                        {{ getAverageRating($pro_user->user->id) }}
    												</span>
    												<span class="opacity-50">
    													({{ getNumberOfReview($pro_user->user->id) }} {{ translate('Reviews') }})
    												</span>
    											</div>
		                                    </div>
		                                </a>
