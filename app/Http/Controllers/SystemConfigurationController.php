...
class SystemConfigurationController extends Controller
{
    ...
    public function update(Request $request)
    {
        if($request->field_name!=null){
            $dt = Faqs::where('type',$request->default_type)->delete();
        }
        foreach($request->field_name ?? [] as $key => $field_name) {
          
           if(!empty($field_name)){
                    $Faqs = new Faqs;
                    $Faqs->title=$field_name;
                    $Faqs->description= $request->description[$key];
                    $Faqs->type = $request->default_type;
                    $Faqs->save();               
            } 
        }
        foreach ($request->types as $key => $type) {
    ...
    public function home_settings(Request $request){
        ...
        return view('admin.default.website.home',compact('faqs'));
    }
    public function brand_settings(Request $request){
        $faqs=Faqs::where('type','brand')->get();
        return view('admin.default.website.brand',compact('faqs'));
    }
    ...
    public function bid_save(Request $request){
        $set_bid_price = SystemConfiguration::where('type','set_bid_price')->first();
        $set_bid_price->value = $request->set_bid_price;
        $set_bid_price->save();
        flash("Bid price has been updated successfully")->success();
        return back();
    }
    public function bid_settings(){      
        return view('admin.default.system_configurations.bid_price_setting.index');
        
    }
    public function payment_settings(){      
        return view('admin.default.system_configurations.payment_service_setting.index');
    }
}
