...
class StripePaymentController extends Controller
{
        ...
        try{
            ...
            elseif (Session::get('payment_data')['payment_type'] == 'wallet_payment') {
                EmailUtility::send_email(
                    ...
                    "The funds are now available in your CashPost wallet.",
                    //. Auth::user()->name,
                    get_email_by_user_id(Auth::user()->id),
                );
    ...
    public function StripeBankAccount(Request $request){  
        ...
        $max_date=Carbon::now()->subYears(18)->format('d-m-Y');
        \Stripe\Stripe::setApiKey(env('STRIPE_SECRET'));
        \Stripe\Stripe::setApiVersion('2020-08-27');
        $user = Auth::user();
        $user_profile = $user->profile;
        ...
        if($prev_url != 'https://cashpost.net/profile' && $prev_url != 'https://cashpost.net/stripe/add-account'){
            flash('You must add a payout method in order to bid/accept Campaigns.')->error();
    ...
    public function StripeAccount(Request $request){  
        ...
        try {
            ...
            $stripeAccount = \Stripe\Account::create([
                ...
                'business_profile'=>[
                        'url' => 'https://cashpost.visionvivante.com',
                        'mcc' => '7311',
                        'support_address' =>[
                            'city' => $request->city,
                            'state' => $request->state,
            ...
            // echo "<pre>code";print_r($e->getError()->code);
            // echo "<pre>param=";print_r($e->getError()->param);
            // echo "<pre>all=";print_r($e->getError()->message);
            // die;
            switch ($e->getError()->code) {
                ...
                case 'postal_code_invalid':
                    ...
                    break;  
                case 'ssn_last_4_invalid':
                    $validator->getMessageBag()->add('ssn_last_4', $e->getError()->message);
                    break;  
                case 'dob_invalid':
                    $validator->getMessageBag()->add('dob', $e->getError()->message);
                    break;    
            ...
            switch ($e->getError()->param) {
                ...
                case 'individual[id_number]':
                    $validator->getMessageBag()->add('id_number', $e->getError()->message);
                    break;  
                ...
                default:
                    // dd($e);
                    flash(translate("There is some error please check with admin of website"))->error();   
                    // $validator->getMessageBag()->add('account_number', $e->getError()->message);
                    break;
            }
        } 
    ...
    public function transfer($active_project){
        ...
        try{
            ...
            flash(translate("Some error with influencer's stripe account, please contact admin"))->error();    
            return false;        
        }
    }
}
