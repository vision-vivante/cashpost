...
class ChatController extends Controller
{
    ...
    public function chat_index(Request $request)
    {
        ...
        if($request->project){
            $project = Project::withTrashed()->where('slug', $request->project)->first();
            
            $receiver = $request->receiver;
            $single_chat_thread = ChatThread::where('project_id',$project->id);
            if (isClient()) {
                $single_chat_thread = $single_chat_thread->where('sender_user_id', Auth::user()->id)->where('receiver_user_id', $receiver);
            }else{
                $single_chat_thread = $single_chat_thread->where('receiver_user_id', Auth::user()->id);
            }
            $single_chat_thread = $single_chat_thread->first();
            ...
            return view('frontend.default.user.messages', compact('single_chat_thread','project'));
        }else{
            abort(404);
        }
    }
